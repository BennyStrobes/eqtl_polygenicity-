...
def initialize_data(gene_df, variance_grid):
	...
	for gene_iter in range(num_genes):
		...
		Y_obs_t = np.load(gene_expr_file)
		num_snps = G.shape[1]

		gene_to_Y_mu[gene_name] = Y_obs_t
		gene_to_Y_var[gene_name] = np.ones(len(Y_obs_t))
	...
	imputation_precision = 1.0

	return gene_to_beta_mu, gene_to_beta_var, gene_to_Z, gene_to_residual_precision, gene_to_Y_mu, gene_to_Y_var, delta_alpha, imputation_precision

def update_beta_and_Z(Y_mu, X, beta_mu, Z, variance_grid, expected_ln_pi, residual_precision, per_snp_h2):
	resid = Y_mu - np.dot(X, beta_mu)
	P = X.shape[1]
...
def update_Y(residual_precision, imputation_precision, G, beta_mu, Y_obs):
	a_term = -.5*(residual_precision + imputation_precision)
	b_term = (residual_precision*np.dot(G,beta_mu)) + (imputation_precision*Y_obs)

	Y_var = -1.0/(2.0*a_term)*np.ones(len(Y_obs))
	Y_mu = b_term*Y_var

	return Y_mu, Y_var


def update_beta_and_Z_shell(gene_df, gene_to_beta_mu, gene_to_Z, gene_to_residual_precision, gene_to_Y_mu, gene_to_Y_var, imputation_precision, variance_grid, expected_ln_pi, gene_heritability_normalization):
	num_genes = gene_df.shape[0]
	...
	for gene_iter in range(num_genes):		
		...
		# Load in data
		Y_obs = np.load(gene_expr_file)
		Y_mu = gene_to_Y_mu[gene_name]
		Y_var = gene_to_Y_var[gene_name]
		#num_snps = len(Y)
		...
		if gene_heritability_normalization == 'True':
			beta_mu, Z, beta_squared = update_beta_and_Z(Y_mu, G, gene_to_beta_mu[gene_name], gene_to_Z[gene_name], variance_grid*per_snp_h2, expected_ln_pi, gene_to_residual_precision[gene_name], per_snp_h2)
		elif gene_heritability_normalization == 'False':
			beta_mu, Z, beta_squared = update_beta_and_Z(Y_mu, G, gene_to_beta_mu[gene_name], gene_to_Z[gene_name], variance_grid, expected_ln_pi, gene_to_residual_precision[gene_name], per_snp_h2)
		else:
			print('gene heritability method currently not implemented')
			pdb.set_trace()

		residual_precision = update_residual_precision_v4(Y_mu, Y_var, G, beta_mu, beta_squared)

		Y_mu, Y_var = update_Y(residual_precision, imputation_precision, G, beta_mu, Y_obs)

		gene_to_beta_mu[gene_name] = beta_mu
		gene_to_Z[gene_name] = Z
		gene_to_residual_precision[gene_name] = residual_precision
		gene_to_Y_mu[gene_name] = Y_mu
		gene_to_Y_var[gene_name] = Y_var



	return gene_to_beta_mu, gene_to_Z, gene_to_residual_precision, gene_to_Y_mu, gene_to_Y_var
...
def update_imputation_precision(gene_df, gene_to_Y_mu, gene_to_Y_var):
	num_genes = gene_df.shape[0]

	new_imputation_alpha = 0.0
	new_imputation_beta = 0.0
	...
	for gene_iter in range(num_genes):		
		...
		resid = (np.square(Y_mu) + Y_var) + np.square(Y_obs)
		resid = resid - (2.0*Y_obs*Y_mu)

		new_imputation_alpha = new_imputation_alpha + (len(Y_obs)/2.0)
		new_imputation_beta = new_imputation_beta + (np.sum(resid)/2.0)

	new_imputation_precision = new_imputation_alpha/new_imputation_beta

	return new_imputation_precision



def inference(gene_df, variance_grid, gene_to_beta_mu, gene_to_Z, gene_to_residual_precision, gene_to_Y_mu, gene_to_Y_var, delta_alpha, imputation_precision, gene_heritability_normalization, output_root, max_iters=1000):


	for itera in range(max_iters):
		expected_ln_pi = scipy.special.psi(delta_alpha) - scipy.special.psi(np.sum(delta_alpha))
		gene_to_beta_mu, gene_to_Z, gene_to_residual_precision, gene_to_Y_mu, gene_to_Y_var = update_beta_and_Z_shell(gene_df, gene_to_beta_mu, gene_to_Z, gene_to_residual_precision, gene_to_Y_mu, gene_to_Y_var, imputation_precision, variance_grid, expected_ln_pi, gene_heritability_normalization)

		delta_alpha = update_delta_alpha(gene_df, gene_to_Z, len(delta_alpha))
		imputation_precision = update_imputation_precision(gene_df, gene_to_Y_mu, gene_to_Y_var)
		imputation_variance = 1.0/imputation_precision
		delta_expected = delta_alpha/np.sum(delta_alpha)
		print('ITERATION ' + str(itera))
		print(delta_expected)
		print(imputation_variance)
		...
		if np.mod(itera, 5) == 0:
			...
			np.savetxt(kurtosis_output_file, np.asarray([expected_var, excess_kurtosis, imputation_variance]).astype(str), fmt="%s", delimiter='\t')
...
def local_heritability_inference(Y, G, max_iters=5000, convergence_thresh=1e-8):
	num_snps = G.shape[1]
...
def update_residual_precision_v4(Y_mu, Y_var, G, beta_mu, beta_squared, hyper_param=1e-16):

	#beta_squared = np.square(beta_mu) + beta_var
	beta_pred = np.dot(G, beta_mu)
	G_squared = np.square(G)

	resid = (np.square(Y_mu) + Y_var) + np.dot(G_squared, beta_squared)
	resid = resid - (2.0*Y_mu*beta_pred)
	# Now add terms with interactions between factors
	resid = resid + (beta_pred*beta_pred - np.dot(G_squared, np.square(beta_mu)))

	new_alpha = hyper_param + (len(Y_mu)/2.0)
	new_beta = hyper_param + (np.sum(resid)/2.0)
...
def local_gene_heritability_estimation(gene_df):
	...
	for gene_iter in range(num_genes):
		...
		pi, expected_var, residual_var = local_mixture_model_heritability_inference(Y_stand, G_stand,max_iters=3000, convergence_thresh=1e-8)
		print(pi)
...
gene_to_beta_mu, gene_to_beta_var, gene_to_Z, gene_to_residual_precision, gene_to_Y_mu, gene_to_Y_var, delta_alpha, imputation_precision = initialize_data(gene_df, variance_grid)

inference(gene_df, variance_grid, gene_to_beta_mu, gene_to_Z, gene_to_residual_precision, gene_to_Y_mu, gene_to_Y_var, delta_alpha, imputation_precision, gene_heritability_normalization, output_root)
