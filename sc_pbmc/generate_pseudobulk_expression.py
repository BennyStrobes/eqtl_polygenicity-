...
def get_ordered_individuals_that_we_have_genotype_and_expression_data_for(adata, genotyped_individuals_file, min_cells_per_indi):
	...
	for line in f:
		line = line.rstrip()
		if line not in dicti:
			print('assumption eororor')
			pdb.set_trace()
		num_cells.append(sum(adata.obs['ind_cov'] == line))
		arr.append(line)
	f.close()
	arr = np.asarray(arr)
...
def generate_pseudobulk_expression(adata_subset, sc_expr_mat_subset, ordered_individuals, min_cells_per_indi):
	...
	for individual_index, individual in enumerate(ordered_individuals):
		...
		cells_to_individual_boolean = adata_subset.obs['ind_cov'] == individual
	...
	for individual_index, individual in enumerate(reordered_individuals):
		...
		pseudobulk_expr[individual_index, :] = np.asarray(np.mean(sc_expr_mat_subset[cells_to_individual_boolean,:], axis=0))
...
pseudobulk_expression_dir = sys.argv[3]
gene_annotation_file = sys.argv[4]

# Genotype PC file
genotype_pc_file = processed_genotype_dir + 'clues_immvar_donor_site_filter_merged_independent_plink.eigenvec'
...
genotyped_individuals_file = processed_genotype_dir + 'clues_immvar_donor_site_filter_merged_samples_that_pass_filters.txt'


# Load in precomputed cluster assignments 
cluster_assignments = np.loadtxt(processed_sc_expression_dir + 'scran_normalization_hvg_2000_regress_batch_True_cluster_assignments_at_various_resolutions.txt', dtype=str,delimiter='\t')
# Subset cluster assignments 
cluster_assignments = cluster_assignments[:,:3]
cluster_assignments[0,0] = 'leiden_0'

# Load in (all genes version of ANn data)
input_h5py_file = processed_sc_expression_dir + '_scran_normalized_all_genes.h5ad'
adata = sc.read_h5ad(input_h5py_file)

# Quick hacky fix to cluster_assignments mat
# Use Perez et al assignments fro third bin
cluster_assignments[0,2] = 'perez_et_al_cell_types'
cluster_assignments[1:,2] = np.asarray(adata.obs['cg_cov'])


# Number of clustering_methods
num_clustering_methods = cluster_assignments.shape[1]


# Extract protein coding gene indices
protein_coding_gene_indices = extract_protein_coding_known_autosomal_genes(adata.var, gene_annotation_file)

# Subset adata matrix to protein-coding autosomal
adata = adata[:, protein_coding_gene_indices]
...
# Ordered gene names
ordered_gene_names = adata.var['gene_ids']
...
# Loop through different clustering methods
for cluster_method_num in range(num_clustering_methods):
	print(cluster_method_num)
	# Name of cluster method
	cluster_method_name = cluster_assignments[0, cluster_method_num]
	# Vector of unique cluster assignments
	unique_cluster_assignments = np.sort(np.unique(cluster_assignments[1:, cluster_method_num]))
	...
	for cluster_assignment in unique_cluster_assignments:
		...
		cell_assigned_to_cluster_boolean = cluster_assignments[1:, cluster_method_num] == cluster_assignment
		
		# Generate pseudobulk expression
		min_cells_per_indi = 5
		pseudobulk_expression, num_cells_per_individual, cluster_specific_individuals = generate_pseudobulk_expression(adata[cell_assigned_to_cluster_boolean, :], sc_expr_mat[cell_assigned_to_cluster_boolean,:], ordered_individuals, min_cells_per_indi)
