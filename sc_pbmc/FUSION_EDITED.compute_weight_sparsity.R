...
# Elastic Net
weights.lasso_glmnet = function( genos , pheno, lambdas=c(100, 10, 1, .1, .01, 1e-3, 1e-4, 1e-5)) {
	eff.wgt = matrix( 0 , ncol=1 , nrow=ncol(genos) )
	# remove monomorphics
	sds = apply( genos  , 2 , sd )
	keep = sds != 0 & !is.na(sds)
	glmnet_lasso = glmnet( x=genos[,keep] , y=pheno, lambda=lambdas , alpha=1.0 , intercept=T , standardize=F )

	betas = glmnet_lasso$beta
	sparsity_levels = glmnet_lasso$lambda
	fraction_non_zero = glmnet_lasso$df/sum(keep)


	fraction_non_zero.matrix = matrix(0,nrow=length(lambdas),ncol=1)
	colnames(fraction_non_zero.matrix) = "lasso_regression"
	rownames(fraction_non_zero.matrix) = lambdas

	fraction_non_zero.matrix[,1] = fraction_non_zero

	return( fraction_non_zero.matrix )
}
...
print(models)
for ( mod in 1:M ) {
	if (models[mod] == "lasso_fixed_lambda") {
		fraction_non_zero.matrix = weights.lasso_glmnet( genos$bed , as.matrix(pheno[,3]))
	}		
}


write.table(fraction_non_zero.matrix, file = paste( opt$out , "_fraction_nonzero_weights.txt" , sep='' ), quote = FALSE, sep = "\t")
# save weights, rsq, p-value for each model, and hsq to output
#snps = genos$bim
#save( fraction_non_zero.matrix , snps , cv.performance , hsq, hsq.pv, N.tot , file = paste( opt$out , ".wgt.RDat" , sep='' ) )
# --- CLEAN-UP
